...
class MessageBuilder extends StatefulWidget {
  const MessageBuilder({Key? key,required this.message,this.prevMessage}) : super(key: key);
...
class _MessageBuilderState extends State<MessageBuilder> {
      ...
      children: [
        ...
        Padding(
          ...
          child: Container(
            ...
            alignment: widget.message.self != null && widget.message.self == true ?Alignment.centerRight: Alignment.centerLeft,
            child: Row(
                            ...
                            _messageProvider(widget.message)!,
                               Padding(
                                 ...
                                 child: Row(
                                    ...
                                    children: [
                                      Text(DateFormat.jm().format(DateTime.fromMicrosecondsSinceEpoch(widget.message.createdAt!)),
                                        style: widget.message.self == true ?
                                        InheritedChatTheme.of(context).theme.sentTimeTextStyle:
                                        InheritedChatTheme.of(context).theme.receivedTimeTextStyle
                                      ),
  ...
  bool? _nipGiver(Message currentMessage, Message? prevMessage){
      if (prevMessage!=null && sameDay(currentMessage.createdAt!, prevMessage.createdAt) == true && currentMessage.self == prevMessage.self) {
             return false;
      }
      else{
        return true;
    ...
    Widget _dateProvider(Message currentMessage, Message? prevMessage){
       if (prevMessage!= null && sameDay(currentMessage.createdAt!, prevMessage.createdAt) == false){
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 20.0),
          child: Text(DateFormat('dd MMM yyyy').format(DateTime.fromMicrosecondsSinceEpoch(currentMessage.createdAt!)),style: InheritedChatTheme.of(context).theme.dateHeaderTextStyle,),
        );
      }
    ...
    Widget _statusProvider(Message message){
        switch(message.status){
          case Status.seen:
            return SvgPicture.asset('assets/double_tick.svg',color: Colors.blue,height: 10,width: 10,);
          case Status.delivered:
            return SvgPicture.asset('assets/double_tick.svg',color: Colors.grey,height: 10,width: 10,);
          case Status.error:
            return const Icon(Icons.error_outline,color: Colors.red,size:18);
          case Status.sending:
            return const SizedBox(height: 10,width: 10,child:  CupertinoActivityIndicator(
            ));
          case Status.sent:
            return SvgPicture.asset('asset/single_tick.svg',color: Colors.grey,height: 10,width: 10,);
          default:
            return const SizedBox();
        }
    }
